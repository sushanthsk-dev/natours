...
var displayMap = function displayMap(locations) {
  console.log('LOLL');
  mapboxgl.accessToken = 'pk.eyJ1Ijoic3VzaGFudGhzayIsImEiOiJja2ZjOTZjcjQxZGNtMnpwcGVlZWgwdGc0In0.yN-PsGOvQbuAC7zkac2XwQ';
  var map = new mapboxgl.Map({
    container: 'map',
    style: 'mapbox://styles/sushanthsk/ckfc9ipbj3ci419rwcrggwqg4',
...
var showAlert = function showAlert(type, msg) {
  hideAlert();
  var markup = "<div class=\"alert alert--".concat(type, "\"  style='transition: ease;'>").concat(msg, "</div>");
...
var stripe = Stripe('pk_test_51Ha5HJFVRoSwUU9lWrZDqO2V0alSnJxzCnFn6On3gMjKfM2gvKeTVAdSQ7HWNAF1TAPMm5oXU6DU8kRLB6nTaH9400JSlS8eBn');
...
if (userDataForm) userDataForm.addEventListener('submit', function (e) {
  ...
  form.append('photo', document.getElementById('photo').files[0]); //since file in array that uses first index

  (0, _updateSettings.updateSettings)(form, 'data'); //A  AJAX call using axios will then actually recognize this form here as an object and will work
});
...
if (bookBtn) bookBtn.addEventListener('click', function (e) {
  ...
  (0, _stripe.bookTour)(tourId); //e.target is basically the element which was clicked, so the one that trigged this event listener here to be fired
  // to access tour id
  // - converted into camCase
});
